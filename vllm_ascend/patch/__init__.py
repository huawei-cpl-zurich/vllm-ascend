...
#    Why:
#       Lookup-table dynamic chunk sizing needs to load or build the full-forward
#       profiling table after `model_executor` is ready. In multiprocessing `spawn`
#       mode the child process starts a fresh interpreter, so monkey-patches applied
#       in the parent are lost unless the subprocess entry point re-applies them
#       before any `EngineCore` is created.
#    How：
#       Replace `EngineCore.__init__` to call `scheduler.run_profiling_chunk_init`
#       when present. Replace `EngineCoreProc.run_engine_core` so importing this
#       module in the child re-runs the idempotent patch helper before delegating
...
#       profiling startup without monkey-patching `EngineCore` and the
...
#
# ** 30. File: worker/patch_v2/patch_use_v2_model_runner.py**
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
