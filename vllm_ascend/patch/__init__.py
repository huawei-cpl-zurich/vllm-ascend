...
#    Why:
#       Profiling-based dynamic chunk sizing needs to load or build the startup
#       profiling cache after `model_executor` is ready. In multiprocessing
#       `spawn` mode the child process starts a fresh interpreter, so monkey-patches
#       applied in the parent are lost unless the subprocess entry point re-applies
#       them before any `EngineCore` is created.
#    How：
