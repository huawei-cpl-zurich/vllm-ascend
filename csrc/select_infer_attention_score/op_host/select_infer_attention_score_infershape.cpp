...
/*!
 * \file select_infer_attention_score_infershape.cpp
 * \brief
 */
                                            ...
                                            const char *inputLayoutPtr)
{
    ...
    if (it != LAYOUT_MAP.end()) {
        ...
        if (queryShape->GetDimNum() != queryDim) {
            OP_LOGE("SelectInferAttentionScore", "Layout %s, query's dim(%zu) must be %zu!",
                queryLayout.c_str(), queryShape->GetDimNum(), queryDim);
            return ge::GRAPH_FAILED;
        }
    } else {
        OP_LOGE("SelectInferAttentionScore", "Only support layout BSH, BSND, BNSD, TND, NTD, BNSD_BSND, BSH_BNSD, "
                                   ...
                                   int64_t& b, int64_t& s1, int64_t& n1, int64_t& d1)
{
    if (queryLayout == "BSH") {
        ...
        OP_LOGE("SelectInferAttentionScore", "Layout %s is not supported in GetQueryBSND function!", queryLayout.c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
                                  ...
                                  int64_t& t, int64_t& n1, int64_t& d1)
{
    if (queryLayout == "TND") {
        ...
        OP_LOGE("SelectInferAttentionScore", "Layout %s is not supported in GetQueryTND function!", queryLayout.c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
                                 ...
                                 int64_t numKeyValueHeads)
{
    if (isPageAttention) { // PA场景
        if (valueShape->GetDimNum() == LAYOUT_PA_BBH_DIM_NUMS) {
            ...
            OP_LOGE("SelectInferAttentionScore", "when Page Attention enabled, value's dim should be 3/4/5, but got %zu.",
                valueShape->GetDimNum());
            return ge::GRAPH_FAILED;
        }
    } else { // 非PA场景
        if (valueShape->GetDimNum() != queryShape->GetDimNum()) {
            OP_LOGE("SelectInferAttentionScore", "when Page Attention not enabled, value'dim(%zu) should equal to query's dim(%zu)!",
...
static ge::graphStatus InferShapeSelectInferAttentionScore(gert::InferShapeContext *context)
{
    if (context == nullptr) {
        OP_LOGE("SelectInferAttentionScore", "context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    OP_LOGD(context->GetNodeName(), "Enter SelectInferAttentionScore InferShape impl.");
    // query shape
    ...
    *attentionOutShape = *queryShape;
    ...
    OP_LOGD(context->GetNodeName(), "SelectInferAttentionScore InferShape end.");
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus InferDataTypeSelectInferAttentionScore(gert::InferDataTypeContext *context)
{
    if (context == nullptr) {
        OP_LOGE("SelectInferAttentionScore", "context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    OP_LOGD(context->GetNodeName(), "Enter SelectInferAttentionScore InferDataType impl.");
    ...
    OP_LOGD(context->GetNodeName(), "SelectInferAttentionScore InferDataType end.");
    return GRAPH_SUCCESS;
}

IMPL_OP_INFERSHAPE(SelectInferAttentionScore)
    .InferShape(InferShapeSelectInferAttentionScore)
    .InferDataType(InferDataTypeSelectInferAttentionScore)
