...
static graphStatus FusedInferHostExecuteFunc(OpExecuteContext *host_api_ctx)
{
    ...
    OP_LOGD(host_api_ctx->GetNodeName(), "SelectInferAttentionScore fallback begin, numHeads = %ld, dScaleValue = %lf",
    ...
    apiRet = EXEC_OPAPI_CMD(
        aclnnSelectInferAttentionScoreV4, fiaTensors.query, ge_tenserListKey, ge_tenserListValue, fiaTensors.pseShiftGe,
...
IMPL_OP(SelectInferAttentionScore).OpExecuteFunc(FusedInferHostExecuteFunc).HostInputs({5, 6, 23});
} // namespace fallback
