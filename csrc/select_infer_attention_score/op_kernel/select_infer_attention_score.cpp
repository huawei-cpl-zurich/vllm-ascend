...
/*!
 * \file select_infer_attention_score.cpp
 * \brief
 */
#include "kernel_operator.h"
#include "kernel_operator_list_tensor_intf.h"
#define FIA_ENABLE_MLA
#include "select_infer_attention_score_tilingkey.h"
#include "select_infer_attention_score_v3.cpp"
#include "flash_attention_interface.cpp"

#define FullQuantTiling 15
extern "C" __global__ __aicore__ void select_infer_attention_score(__gm__ uint8_t* query, __gm__ uint8_t* key, __gm__ uint8_t* value,
                                                                ...
                                                                __gm__ uint8_t* attentionOut, __gm__ uint8_t* softmaxLse, __gm__ uint8_t* workspace, __gm__ uint8_t* tiling)
{
    if (TILING_KEY_VAR >= FAI_FLAG_TILING) {
        ...
        // Decode-only port intentionally disables prefill/IFA routing.
        AscendC::printf("Unsupported tiling path in select_infer_attention_score decode-only port.\n");
    }
}
