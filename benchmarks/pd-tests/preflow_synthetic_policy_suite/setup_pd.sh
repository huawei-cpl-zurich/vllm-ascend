...
# Prefill can keep several requests and their KV resident, while the selected
# policy sends exactly one request to compute in each scheduler step.
...
Serving options:
  ...
  --prefill-scheduler-cls CLS  Fully qualified asynchronous scheduler class for
                               the prefill worker.
  --scheduler-config-json JSON Complete vLLM --additional-config JSON shared by
                               the P/D workers. The harness supplies this value.
...
while (($#)); do
    case "$1" in
        ...
        --prefill-scheduler-cls)
            require_option_value "$1" "${2:-}"
            PREFILL_SCHEDULER_CLS="$2"
            shift 2
            ;;
        --scheduler-config-json)
            require_option_value "$1" "${2:-}"
            SCHEDULER_CONFIG_JSON="$2"
            shift 2
            ;;
...
[[ -n "${PREFILL_SCHEDULER_CLS}" ]] || die "--prefill-scheduler-cls is required"
[[ -n "${SCHEDULER_CONFIG_JSON}" ]] || die "--scheduler-config-json is required"
...
for value in "$PREFILL_WORKERS" "$DECODE_WORKERS" "$TP_SIZE" "$PP_SIZE" "$MAX_MODEL_LEN" \
    ...
    "$WORKER_PORT_BASE" "$PREFILL_KV_PORT_BASE" "$DECODE_KV_PORT_BASE"; do
...
scheduler_additional_config() {
    printf '%s' "$SCHEDULER_CONFIG_JSON"
}
...
write_launch_config() {
    {
        ...
        printf 'seed: %s\n' "$SEED"
        printf 'prefill_scheduler_cls: "%s"\n' "$(json_escape "$PREFILL_SCHEDULER_CLS")"
        printf 'scheduler_config_json: "%s"\n' "$(json_escape "$SCHEDULER_CONFIG_JSON")"
...
start_worker() {
    ...
    if [[ "$worker_type" == "prefill" ]]; then
        cmd+=(--scheduler-cls "$PREFILL_SCHEDULER_CLS")
    fi
